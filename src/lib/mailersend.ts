      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        if (responseText.trim()) {
          try {
            ...
            console.warn('Failed to parse JSON response, using empty object:', parseError)
          }
        }
      } else {
        console.log('MailerSend response is not JSON, content-type:', contentType)
      }

      return {
        success: true,
        messageId: (data as any).message_id || 'unknown'
      }
    } catch (error) {
